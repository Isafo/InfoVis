		...
		for (var i = 0; i < data.length; i++) {
				...
				for (var z = 0; z < _centroids.length; z++) {
					...
					keys.forEach (function (d) {
						distance += Number(Math.pow(_centroids[z][d] - cur_value[d], 2));
					});

					distance = Number(Math.sqrt(distance));
   ...
   function moveCentroid(data, _centroids, _centroid_index) {
    ...
    function kmeans(data, k) {
        ...
        //...
		var prev_quality = Number(Infinity);
		var square_dist = Number(Infinity);
		var qu  = Number(Infinity);
		keys = d3.keys(data[0]);
		var distance = 0;
		...
		do {
			...
			for (var i = 0; i < centroid_index.length; i++) {
				...
				for (var z = 0; z < centroids.length; z++) {
					...
					square_dist = Number(Math.pow(distance, 2));
					temp.push(square_dist);

				}
			}

			qu = Number(Math.min.apply(Math, temp));

			console.log("prev: " + prev_quality);
			console.log("new quality: " + qu);

		} while(qu < prev_quality || prev_quality - qu > 0.001);
